...
class AdaptiveOrderManager:
    ...
    async def place_trailing_stop(self, symbol: str, trailing_stop_price: str, active_price: Optional[str] = None, position_idx: int = 0, trigger_direction: Optional[int] = None):
        ...
        try:
            ...
            if is_reset and "10001" in es:
                self.logger.info("TS_ALREADY_REMOVED{symbol=%s, reason='already_removed_10001'}", symbol)
                return {"success": True, "already_set": True}
            if "10001" in es and not is_reset:
                self.logger.error("TS_APPLY_FAIL: Invalid parameters for %s. code=10001, msg='%s'", symbol, es)
                return {"success": False, "error": es, "no_retry": True}
            self.logger.exception("Trailing stop placement error: %s", e)
            return {"success": False, "error": es}
...
class Stage2CopyTradingSystem:
    ...
    async def on_position_item(self, item: dict):
        ...
        try:
            ...
            # Non-blocking Leverage Synchronization
            donor_leverage = item.get('leverage')
            if donor_leverage and self._last_synced_leverage.get(symbol) != donor_leverage:
    ...
    def _is_copy_ready(self) -> bool:
        """
        Копирование разрешено, если:
        - включён режим копирования
        - инициализированы REST клиенты MAIN/SOURCE
        - ключи/лимиты загружены (минимум — не None)
        """
        if not self.copy_enabled:
            return False
        # Базовые проверки (адаптируйте под ваши поля и init):
        if not getattr(self, "main_client", None):
            return False
        if not getattr(self, "source_client", None):
            return False
        return bool(self._copy_ready)
