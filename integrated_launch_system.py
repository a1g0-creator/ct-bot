...
class IntegratedTradingSystem:
    ...
    async def apply_new_credentials(self, account_id: int) -> bool:
        """
        Hot-applies credentials by delegating to the robust hot_swap_credentials method.
        This method now serves as a compatibility wrapper.
        """
        logger.info(f"HSWAP_TRIGGER: apply_new_credentials called for account {account_id}, delegating to hot_swap_credentials.")
        if not self.stage1_monitor:
            logger.error("HSWAP_TRIGGER: Aborted. stage1_monitor is not available.")
            await send_telegram_alert("❌ HSWAP: Ошибка! Система мониторинга не инициализирована.")
            return False

        try:
            # We use the provided account_id as the target, and resolve the donor ID internally.
            _, donor_id = self._resolve_account_ids()
            return await self.stage1_monitor.hot_swap_credentials(
                target_account_id=account_id,
                donor_account_id=donor_id
            )
        except Exception as e:
            logger.critical(f"HSWAP_TRIGGER: Failed to execute hot_swap_credentials from apply_new_credentials: {e}", exc_info=True)
            await send_telegram_alert(f"❌ HSWAP: Критическая ошибка при вызове горячей замены: {e}")
            return False
    ...
    async def _on_keys_saved(self):
        ...
        """
        logger.info("HSWAP_TRIGGER: _on_keys_saved called, delegating to hot_swap_credentials.")
        if not self.stage1_monitor:
            logger.error("HSWAP_TRIGGER: Aborted. stage1_monitor is not available.")
            await send_telegram_alert("❌ HSWAP: Ошибка! Система мониторинга не инициализирована.")
            return
        ...
        try:
            ...
            logger.critical(f"HSWAP_TRIGGER: Failed to execute hot_swap_credentials: {e}", exc_info=True)
            await send_telegram_alert(f"❌ HSWAP: Критическая ошибка при вызове горячей замены: {e}")
